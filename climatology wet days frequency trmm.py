...
def month_start_end(year):
    ...
    if year in [1964, 1968, 1972, 1976, 1980, 1984, 1988, 1992, 1996, 2000,
        ...
        month_start_end_list = [(0, 31), (31, 60), (60, 91), (91, 121),
                                (121, 152), (152, 182), (182, 213), (213, 244),
                                (244, 274), (274, 305), (305, 335), (335, 366)]
...
def file_name(year):
    return INPUT_FOLDER + model.name + '/' + model.version + '-region-' + \
        str(year) + '.nc4'
...
Model = namedtuple('Model', 'name version lat lon precip plot_pos')
XYZ = [141, 142, 143, 144]
trmm = Model('trmm', 'TRMM_3B42', 'lat', 'lon', 'precipitation', XYZ[0])
chirps = Model('chirps', 'chrips-v2', 'latitude', 'longitude', 'precip', XYZ[1])
model = trmm
...
title = model.name
plot_filename = 'climatology-wet-days-frequency-' + SEASON_NAME + '-' + \
    str(YEARS[0]) + '-' + str(YEARS[-1]) + '-' + title
