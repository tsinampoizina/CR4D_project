...
def compute_dry_spell_freq_year_season(year, season):
    ...
    dry_spell_len_zeroed = xr.where(dry_spell_len == length(year, season.months),0,dry_spell_len)
    dry_spells = xr.merge([dry_spell_count, dry_spell_len_zeroed])
    return dry_spells
...
for season in seasons:
   ...
   if season == djfm:
       ...
       dry_spell_ave_len = Characteristic('DRY-SPELL-LENGTH', np.arange(0, 15, 1, dtype=float),
                               'spells', 5, 0, 1, np.arange(0, 19, 1, dtype=float))
   if season == all_year:
       dry_spell_freq = Characteristic('DRY-SPELL-FREQUENCY', np.arange(0, 30, 1, dtype=float),
                               'spells', 5, 0, 1, np.arange(0, 30, 1, dtype=float))
       dry_spell_ave_len = Characteristic('DRY-SPELL-LENGTH', np.arange(0, 120, 1, dtype=float),
                               'spells', 5, 0, 1, np.arange(5, 120, 5, dtype=float))
   ...
   for model in MODELS:
      ...
      title = model.name + ' '  +  charac.name + ' ' + season.name + ' ' + str(YEARS[0]) + '-' + str(YEARS[-1])

      t0 = time.time()

      climatology_freq = compute_climatology_season(YEARS, season)
      climatology_freq = climatology_freq / len(YEARS)
      #if model.name == 'trmm' or model == ichirps:
