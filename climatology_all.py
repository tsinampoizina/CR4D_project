...
def countour_plot(dat, contour_levels, title):
    ...
    bottom_plots = range(nplots-model.plot_pos[1]+1, nplots)
...
def get_precip(year):
    ...
    prec = dsx[model.precip]
    if model == tamsat:
        dsx = xr.open_dataset(file)
        prec = dsx['rfe']

    if model.precip != 'pr' and model != tamsat:
    ...
    if model.name == 'arc2':
        ...
        prec = prec.transpose('lat', 'lon', 'time')
    if model.precip == 'pr':
...
def compute_climatology_season(years, season):
    if model.name == 'ens':
        return compute_climatology_cordex_ens(years, season)
    if charac.name == 'DRY-SPELL-LENGTH':
        return compute_climatology_dry_spell(years, season, 'dry_spell_ave_len')
    if charac.name == 'DRY-SPELL-FREQUENCY':
        return compute_climatology_dry_spell(years, season, 'dry_spell_freq')
    ...
    for year in years:
        ...
        if Path(path_data_file).is_file() and not save_data_file:
           ds_disk = xr.open_dataset(path_data_file)
...
def compute_climatology_cordex_ens(years, season):
    CORDEX_MODELS = [remo, clm, rm3p, gem3, hihram, crcm5, racmo, rca4]
    for cordex_model in CORDEX_MODELS:
        for year in years:
            PLOT_DATA_FOLDER =  PLOT_DATA_FOLDER_ROOT + cordex_model.name + '/'
            path_data_file = PLOT_DATA_FOLDER + season.name + str(year) + '.nc'
            path_data_file_prev = PLOT_DATA_FOLDER + season.name + str(year-1) + '.nc'
            if Path(path_data_file).is_file():
               ds_disk = xr.open_dataset(path_data_file)
               this_year = ds_disk[charac.variable_name]
            else:
                print(path_data_file, "not found")
            if year == years[0]:
               climatology = xr.zeros_like(this_year)
            this_year = this_year.assign_coords(lat=climatology.lat)
            this_year = this_year.assign_coords(lon=climatology.lon)
        ...
        if charac == average_daily:
            climatology = climatology/length(year, season.months)
        if cordex_model == CORDEX_MODELS[0]:
            ens_climatology = xr.zeros_like(climatology)
        print(cordex_model.name, climatology.dims, 'coords', climatology.coords)
        if 'lat' in climatology.dims:
            print('in this case', climatology.dims)
            climatology = climatology.transpose('lon','lat')
        ...
        ens_climatology += climatology
    return ens_climatology/(len(CORDEX_MODELS))
        

def file_name(year):
    if model.precip == 'pr':    # Cordex people
        file = 'pr_AFR-44_ECMWF-ERAINT_evaluation_r1i1p1_' + model.version + '_day_'
        folder = '/media/sr0046/WD-exFAT-50/CORDEX/evaluation/' + model.version +\
...
for season in seasons:
   ...
   #MODELS = [ichirps, gpcc]
   #MODELS = [clm]
   MODELS += [remo, clm, rm3p, gem3, hihram, crcm5, racmo, rca4, ens]
   #MODELS = [hihram, crcm5, racmo, rca4, ens]
   ...
   for model in MODELS:
      ...
      if model.name == 'arc2':
         YEARS = range(2002, 2003) # 2002 to 2009
      else:
         YEARS = range(1999, 2008)  # 2005 izao
      ...
      indiv_title = model.name
      title = charac.name + ' ' + season.name + ' ' + str(YEARS[0]) + '-' + str(YEARS[-1])
   ...
   if save_plot_file:
       ...
       if len(MODELS) == 9:
           ...
           data_grid_or_cordex = sum([model.name for model in MODELS])
       else:
           data_grid_or_cordex = ''
